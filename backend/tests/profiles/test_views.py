...
def test_login_with_data(user_payload: dict) -> None:
    ...
    assert len(Profile.objects.all()) == 1
    created_user = dict(email=user_payload['email'],
...
@pytest.mark.django_db
def test_detail_another_user_profile_logged_in(user_payload: dict,
                                               user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    client.post(reverse('register'), user_payload, format='json')
    client.post(reverse('register'), user2_payload, format='json')
    assert len(Profile.objects.all()) == 2
    created_user = dict(email=user2_payload['email'],
...
@pytest.mark.django_db
def test_update_view_logged_in_as_owner_put_request(
        user_payload: dict, temporary_image: bytes) -> None:
    assert len(Profile.objects.all()) == 0
    client.post(reverse('register'), user_payload, format='json')
    assert len(Profile.objects.all()) == 1
    created_user = dict(email=user_payload['email'],
    ...
    update_profile = dict(email=user_payload['email'],
                          username=user_payload['email'].split('@')[0],
                          image=temporary_image)
    put_res = client.put(reverse('profile_update', kwargs={'pk': 1}),
                         update_profile,
                         format='multipart')
...
@pytest.mark.django_db
def test_update_view_logged_in_as_owner_patch_request(
        user_payload: dict, temporary_image: bytes) -> None:
    assert len(Profile.objects.all()) == 0
    client.post(reverse('register'), user_payload, format='json')
    assert len(Profile.objects.all()) == 1
    ...
    assert get_res.data['username'] == user_payload['email'].split('@')[0]
    assert get_res.data['image'] == None
    update_profile = dict(image=temporary_image)
    put_res = client.patch(reverse('profile_update', kwargs={'pk': 1}),
                           update_profile,
                           format='multipart')
    assert put_res.data['id'] == 1
    assert put_res.data['email'] == user_payload['email']
...
# friend request
@pytest.mark.django_db
def test_friend_request_not_logged_in(user_payload: dict,
                                      user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
    assert len(Profile.objects.all()) == 2
    req = client.post(reverse('send_friend_request',
                              kwargs={
                                  'sender_pk': user1.data['id'],
                                  'receiver_pk': user2.data['id']
                              }),
                      format='json')
    assert req.data[
        'detail'] == 'Authentication credentials were not provided.'
...
@pytest.mark.django_db
def test_friend_request_already_created_by_user1(user_payload: dict,
                                                 user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
    ...
    created_user = dict(email=user_payload['email'],
                        password=user_payload['password'])
    client.post(reverse('login'), created_user, format='json')
    req_payload = dict(sender=user1.data['id'], receiver=user2.data['id'])
    req = client.post(reverse('send_friend_request',
                              kwargs={
                                  'sender_pk': user1.data['id'],
                                  'receiver_pk': user2.data['id']
...
@pytest.mark.django_db
def test_friend_request_already_created_by_user2(user_payload: dict,
                                                 user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
    ...
    created_user = dict(email=user2_payload['email'],
                        password=user2_payload['password'])
    client.post(reverse('login'), created_user, format='json')
    req_payload = dict(sender=user2.data['id'], receiver=user1.data['id'])
    req = client.post(reverse('send_friend_request',
                              kwargs={
                                  'sender_pk': user2.data['id'],
                                  'receiver_pk': user1.data['id']
                              }),
                      req_payload,
                      format='json')
...
@pytest.mark.django_db
def test_friend_request_with_data_logged_in(user_payload: dict,
                                            user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
...
@pytest.mark.django_db
def test_friend_request_with_no_data_logged_in(user_payload: dict,
                                               user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
...
@pytest.mark.django_db
def test_friend_request_accept_loggen_in_as_receiver(
        user_payload: dict, user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
    assert len(Profile.objects.all()) == 2
    ...
    created_user = dict(email=user_payload['email'],
                        password=user_payload['password'])
    client.post(reverse('login'), created_user, format='json')
    req = client.get(reverse('accept_friend_request',
                             kwargs={
                                 'user_pk': user1.data['id'],
                                 'f_req_pk': f_req.id
                             }),
                     format='json')
    assert req.data[
        'message'] == f'{f_req.sender.username} added to your friend list.'
...
@pytest.mark.django_db
def test_friend_request_accept_not_loggen_in(user_payload: dict,
                                             user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
...
@pytest.mark.django_db
def test_friend_request_accept_loggen_in_not_receiver(
        user_payload: dict, user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
...
@pytest.mark.django_db
def test_friend_request_refuse_loggen_in_as_receiver(
        user_payload: dict, user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
    ...
    created_user = dict(email=user_payload['email'],
                        password=user_payload['password'])
    client.post(reverse('login'), created_user, format='json')
    req = client.get(reverse('refuse_friend_request',
                             kwargs={
                                 'user_pk': user1.data['id'],
                                 'f_req_pk': f_req.id
                             }),
...
@pytest.mark.django_db
def test_friend_request_refuse_not_loggen_in(user_payload: dict,
                                             user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
...
@pytest.mark.django_db
def test_friend_request_refuse_loggen_in_not_receiver(
        user_payload: dict, user2_payload: dict) -> None:
    assert len(Profile.objects.all()) == 0
    user1 = client.post(reverse('register'), user_payload, format='json')
    user2 = client.post(reverse('register'), user2_payload, format='json')
...
def test_remove_friend_not_logged_in() -> None:
    res = client.get(reverse('remove_friend',
                             kwargs={
                                 'user_pk': 1,
                                 'friend_pk': 2
                             }),
...
@pytest.mark.django_db
def test_remove_friend_logged_in_not_one_of_friends(
        user_payload: dict, user2_payload: dict, user3_payload: dict) -> None:
    ...
    created_user = dict(email=user3_payload['email'],
...
@pytest.mark.django_db
def test_remove_friend_logged_in_not_profile_owner(
        user_payload: dict, user2_payload: dict) -> None:
    u1 = client.post(reverse('register'), user_payload, format='json')
    u2 = client.post(reverse('register'), user2_payload, format='json')
    user1 = Profile.objects.get(id=u1.data['id'])
...
@pytest.mark.django_db
def test_remove_friend_logged_in_profile_owner(user_payload: dict,
                                               user2_payload: dict) -> None:
    u1 = client.post(reverse('register'), user_payload, format='json')
    u2 = client.post(reverse('register'), user2_payload, format='json')
    user1 = Profile.objects.get(id=u1.data['id'])
    ...
    res = client.get(reverse('remove_friend',
                     ...
                     format='json')
    assert res.data[
        'message'] == f"{user2.username} was deleted from friends list"
    assert res.status_code == 200
...
def test_friend_list_not_logged_in(user_payload: dict) -> None:
    client.post(reverse('register'), user_payload, format='json')
    res = client.get(reverse('friends_list', kwargs={'user_pk': 1}),
                     format='json')
    assert res.data[
        'detail'] == 'Authentication credentials were not provided.'
    assert res.status_code == 403
...
@pytest.mark.django_db
def test_friend_list_logged_in_as_user1(user_payload: dict,
                                        user2_payload: dict) -> None:
    u1 = client.post(reverse('register'), user_payload, format='json')
    u2 = client.post(reverse('register'), user2_payload, format='json')
    user1 = Profile.objects.get(id=u1.data['id'])
    ...
    created_user = dict(email=user_payload['email'],
                        password=user_payload['password'])
    client.post(reverse('login'), created_user, format='json')
    res = client.get(reverse('friends_list', kwargs={'user_pk': user1.pk}),
                     format='json')
...
@pytest.mark.django_db
def test_friend_list_logged_in_as_user2(user_payload: dict,
                                        user2_payload: dict) -> None:
    u1 = client.post(reverse('register'), user_payload, format='json')
    u2 = client.post(reverse('register'), user2_payload, format='json')
    user1 = Profile.objects.get(id=u1.data['id'])
...
@pytest.mark.django_db
def test_profiles_list_logged_in(user_payload: dict,
                                 user2_payload: dict) -> None:
    client.post(reverse('register'), user_payload, format='json')
    client.post(reverse('register'), user2_payload, format='json')
    created_user = dict(email=user_payload['email'],
